...
document.addEventListener("DOMContentLoaded", (e) => {
  document.querySelector("#ligaId").addEventListener("change", updateNombre);
});
...
function mainTabs() {
  ...
  function resetFormQuiniela() {
    ...
    jQuery("#tituloFormQuiniela").html(
...
function buttons() {
    ...
    jQuery.ajax({
      ...
      success: function (json) {
        if (json.status == "OK") {
          ...
          jQuery("#tituloFormQuiniela").html(
            '<i class="fas fa-edit me-2"></i>Editar Quiniela',
          );
  ...
  jQuery("body").on("click", ".btnEliminarQuiniela", function (e) {
    ...
    if (
      !confirm(
        "¿Estás seguro de que deseas eliminar esta quiniela? Esta acción no se puede deshacer.",
      )
    ) {
      return;
    }
  ...
  jQuery("body").on("click", ".btnNuevaQuiniela", function (e) {
    ...
    if (validator.validate("#formNuevaQuiniela")) {
      var quinielaId = jQuery("#quinielaId").val();
      var url = quinielaId
        ? "/quinielas/mis-quinielas/updateQuiniela"
        : "/quinielas/mis-quinielas/saveQuiniela";
      var mensajeExito = quinielaId
        ? "Quiniela actualizada con éxito"
        : "Quiniela creada con éxito";
...
function updateNombre() {
  ...
  const $ligaId = document.getElementById("ligaId");

  if ($ligaId && $nombre) {
    $nombre.value = $ligaId.options[$ligaId.selectedIndex].text;
    console.log("Nombre actualizado:", $nombre.value);
  }
}
